...
GraphNode* GraphNode_insert(
  ...
  // Insert at bottom and color it red.
    if (h == NULL) { return create_nodeGT(key, data, 1); }
    int cmp = strcmp(key, h->key);
    if (cmp < 0) { h->l = GraphNode_insert(h->l, key, data, update, update_func); }
    else if (cmp > 0) { h->r = GraphNode_insert(h->r, key, data, update, update_func); }
    else if (update) {update_func(h->data, data);}
  // Lean left.
    if (is_redGT(h->r) && !is_redGT(h->l)) { h = rotate_leftGT(h); }
  // Balance 4-node.
    if (is_redGT(h->l) && is_redGT(h->l->l)) { h = rotate_rightGT(h);}
  // Split 4-node.
    if (is_redGT(h->l) && is_redGT(h->r)) { flip_colorsGT(h); }
    return h;
}
